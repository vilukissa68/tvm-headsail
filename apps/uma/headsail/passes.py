...
# under the License.
"""Transform passes for the headsail accelerator"""
...
class HeadsailConv2dPass:
    _EXTERNAL_FUNCTION_NAME = "dla_conv2d_bias"
        ...
        def _detect_and_replace_conv2d(
            ...
            def _replace_conv2d(op):
                ...
                if op == _entry_node:
                    ...
                    input_offsets = input_offsets + ["HWC"] # Input order

                    # Kernel params
                    kernel_args = ["co", "ci", "kw", "kh"]
                    kernel_offsets = [_loops[i].extent.value for i in kernel_args]
                    kernel_offsets = kernel_offsets + ["KCHW"] # Input order

                    # Padding
                    bias_args = ["co"]
                    bias_length = [_loops[i].extent.value for i in bias_args]
                    padding = [0, 0, 0, 0, 0]
                    stride = [1, 1]
                    clips = [0, 0]
...
def tir_call(ib: tvm.tir.ir_builder, extern: bool, name: str, *args):
    ...
    *args:
    ...
    def buf_from_array(ib, arr, dtype):
        ...
        var = ib.allocate("int8", (len(arr),), scope="global")
    ...
    if extern:
        ...
        return tvm.tir.call_extern("int8", name, *args)
    else:
        print("No Extern")
        args = [
            buf_from_array(ib, i, "int8")
